...
func _on_body_entered(body: Node2D) -> void:
	if body.name == "Luna":
		%SlideViewer.open("res://Slides/Portal4", true, false, self)
...
func _on_slide_viewer_classify_requested(texto: String) -> void:
	...
	if err != OK:
		print("Oracle1 classifier: request() failed to start, error code %d" % err)
...
func _on_classifier_request_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		print("Oracle1 classifier: request failed - result=%d response_code=%d body=%s" % [result, response_code, body.get_string_from_utf8()])
		%SlideViewer.set_classification_result("No se pudo conectar con el modelo. Intenta de nuevo.")
		return

	var json := JSON.new()
	if json.parse(body.get_string_from_utf8()) != OK:
		print("Oracle1 classifier: JSON parse failed - response_code=%d body=%s" % [response_code, body.get_string_from_utf8()])
		%SlideViewer.set_classification_result("No se pudo conectar con el modelo. Intenta de nuevo.")
		return

	var data = json.data
	if typeof(data) != TYPE_DICTIONARY or not data.has("label") or not data.has("confidence"):
		print("Oracle1 classifier: response missing expected fields - response_code=%d body=%s" % [response_code, body.get_string_from_utf8()])
		%SlideViewer.set_classification_result("No se pudo conectar con el modelo. Intenta de nuevo.")
		return
